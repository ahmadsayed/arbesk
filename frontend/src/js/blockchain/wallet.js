...
function initWallet() {
    ...
    if (!hasWeb3Modal) {
        console.warn('Web3Modal not loaded; direct MetaMask fallback will be used on connect.');
        return;
    }
...
async function _initContract() {
    try {
        const [addrRes, abiRes] = await Promise.all([
            ...
            fetch('/api/abi/ArbeskWorld.json')
        ]);
    ...
    try {
        ...
        if (switchError.code === 4902) {
            try {
                ...
                alert('Please add the Hardhat Local network manually:\nRPC: http://127.0.0.1:8545\nChain ID: 31415822');
                return false;
            }
...
/**
 * Pay for a generation using the ArbeskWorld contract.
...
async function payForGeneration(nodeId, prompt) {
    ...
    if (chainId !== HARHAT_CHAIN_ID_DEC) {
        const ok = confirm(
            'You are not on the Hardhat Local network (chain ' + chainId + ').\n\n' +
    ...
    try {
        ...
        const nodeIdBytes32 = w3.utils.padRight(
            w3.utils.utf8ToHex(nodeId),
            64
        );
        ...
        } else if (msg.includes('User denied') || msg.includes('rejected') || error.code === 4001) {
...
async function mintWorld(tokenURI, tokenId) {
    const c = _getContract();
    const w3 = _getWeb3();
    ...
    try {
        const tx = c.methods.mintWorld(tokenURI, tokenId);
        ...
        document.dispatchEvent(new CustomEvent('wallet:worldMinted', {
            detail: { tokenId, tokenURI, txHash: receipt.transactionHash }
        }));

        return receipt.transactionHash;
    } catch (error) {
        console.error('mintWorld failed:', error);
        return null;
    }
}
...
async function updateTokenURI(tokenId, newTokenURI) {
    ...
    try {
        const tx = c.methods.updateTokenURI(tokenId, newTokenURI);
        const gas = await tx.estimateGas({ from: window.walletAddress });
        const receipt = await tx.send({
            from: window.walletAddress,
            gas: Math.floor(Number(gas) * 1.2)
        ...
        console.error('updateTokenURI failed:', error);
        return null;
    }
}
...
window.mintWorld = mintWorld;
window.updateTokenURI = updateTokenURI;
