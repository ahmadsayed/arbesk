...
async function loadEthereumProvider() {
  ...
  try {
    ...
    console.warn("[WALLET-CONNECT] esm.sh load failed:", (err as Error).message);
  }
  ...
  try {
    ...
    console.warn("[WALLET-CONNECT] jsdelivr load failed:", (err as Error).message);
  }
 ...
 */
export function offWalletConnectEvent(
  event: string,
  handler: (...args: any[]) => void
) {
  if (!provider) return;
