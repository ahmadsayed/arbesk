...
async function _promptNetworkSwitch(networkKey: string): Promise<boolean> {
    ...
    if (_isChainUnknownError(switchError)) {
      ...
      try {
        ...
        console.warn(
          ...
          (addError as Error).message || addError
        );
        showToast({
          type: "warning",
      ...
      showToast({
        ...
        message: (switchError as Error).message || "Could not switch network.",
        duration: 0,
      });
      return false;
    }
