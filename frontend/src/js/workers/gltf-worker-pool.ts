...
// worker cannot use top-level `import`/`export`, so gltf-worker.ts - an ES
...
export function getGlTFWorkerPool(): any {
  if (!pool) {
    pool = createPool();
  }
  return pool;
...
export async function isWorkerPoolAvailable(): Promise<boolean> {
  ...
  try {
    ...
    const missing = REQUIRED_WORKER_METHODS.filter((m: string) => !methods.includes(m));
    if (missing.length === 0) {
      available = true;
      return true;
    }
    ...
    console.warn("[WORKER-POOL] module workers not available, falling back to main thread:", (error as Error).message);
  }
...
export function terminateGlTFWorkerPool(): void {
  if (pool) {
    pool.terminate().catch(() => {});
    pool = null;
  }
