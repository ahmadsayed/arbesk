...
import * as store from "../domain/version-history-store.ts";
...
/** Plain {x,y,z} — inputs may be plain objects or BABYLON.Vector3 instances. */
...
export function _indexForAngle(angleDeg: number, n: number): number {
  if (n === 0) return -1;
 ...
 */
export function _lerpAngle(from: number, to: number, t: number): number {
  let d = to - from;
 ...
 */
function utilityScene(mainScene: BABYLON.Scene): BABYLON.Scene {
  return (
...
function placeHandle(g: any, angleRad: number): void {
...
function buildGizmoForNode(scene: BABYLON.Scene, nodeId: string): any {
  ...
  const gizmo: any = { nodeId, anchor, dragHoverIdx: -1 };
...
function syncVisuals(g: any): void {
  const s = store.getState();
  const activeIdx = g.filtered.findIndex(
    (e: any) => e.cid === s.activeCid
  );
  const safeIdx = activeIdx >= 0 ? activeIdx : g.filtered.length - 1;
...
function wireDrag(
  ...
  function ringAngleFromPointer(): number | null {
 ...
 */
export function initModelClockGizmo(
  scene: BABYLON.Scene,
  camera: BABYLON.ArcRotateCamera
): () => void {
  ...
  let clockTargetNodeId: string | null = null;
   ...
   */
  function positionLabelEl(el: HTMLElement, world: BABYLON.Vector3): void {
  ...
  function createTickLabels(gizmo: any): void {
    if (!tickLabelsContainer) return;
    gizmo.tickLabelEls = gizmo.filtered.map((entry: any) => {
  ...
  function onModeChanged(e: any): void {
    if (e?.mode === "time") {
  ...
  function onKeyDown(e: KeyboardEvent): void {
    ...
    const activeIdx = current.filtered.findIndex(
      (entry: any) => entry.cid === store.getState().activeCid
    );
    switch (e.key) {
      case "ArrowLeft":
      case "ArrowDown":
