/**
 * Manifest History Browser — Timeline Scrubber
 *
 * Renders a horizontal draggable timeline of manifest version nodes.
...
import { clearScene, loadManifest } from '../engine/scene-graph.js';
import { contract } from '../blockchain/wallet.js';

// ─── DOM References ───
const historySection = document.getElementById('manifestHistory');
const historyBar = historySection ? historySection.querySelector('.history-bar') : null;
...
async function _fetchChain(cid) {
    if (!cid) return [];
    try {
        const res = await fetch(`/api/manifest-chain?cid=${encodeURIComponent(cid)}`);
        if (!res.ok) throw new Error('Failed to fetch manifest chain');
...
async function _fetchPublishedCid() {
    const tokenId = window.activeTokenId;
...
function _render() {
    ...
    for (const entry of chainCache) {
        ...
        node.title = `${entry.name || 'Untitled'} • v${entry.version} • ${entry.nodeCount} node${entry.nodeCount !== 1 ? 's' : ''}${entry.timestamp ? ' • ' + new Date(entry.timestamp).toLocaleString() : ''}`;
...
async function _onNodeClick(cid) {
    ...
    try {
        ...
        isLoading = false;
        setTimeout(() => { isHistoryNavigation = false; }, 100);
    }
}

async function _refresh() {
    const manifestCid = window.activeManifestId;
    if (!manifestCid) {
        chainCache = [];
        chainRootCid = null;
        activeCid = null;
...
function _initDragScroll() {
    ...
    historyBar.addEventListener('touchstart', (e) => {
        if (e.target.closest('.history-node')) return;
        isDragging = true;
    ...
    historyBar.addEventListener('touchmove', (e) => {
        if (!isDragging) return;
...
document.addEventListener('scenegraph:ready', (e) => {
    const manifestCid = e.detail?.manifestCid || window.activeManifestId;
    ...
    chainRootCid = manifestCid;
    activeCid = manifestCid;
    window.latestManifestId = manifestCid;
    _refresh();
});

document.addEventListener('wallet:connected', () => {
    if (window.activeManifestId && !isHistoryNavigation) {
        _refresh();
    }
});

document.addEventListener('wallet:worldMinted', () => {
    // Re-check published CID after mint/update
    setTimeout(_refresh, 500);
});

document.addEventListener('manifest:saved', () => {
...
if (window.activeManifestId) {
    chainRootCid = window.activeManifestId;
    _refresh();
}
