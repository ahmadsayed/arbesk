...
export interface CollaboratorPanelState {
  tokenId: string | number;
  editable: boolean;
  isOwner: boolean;
  abortController: AbortController;
  ownerBadge?: HTMLElement;
  list?: HTMLElement;
...
export function initCollaboratorPanel(
  ...
  const state: CollaboratorPanelState = {
    tokenId,
...
function renderList(state: CollaboratorPanelState, editorList: any[]): void {
  const { list, ownerBadge, controls } = state as Required<CollaboratorPanelState>;
  list.innerHTML = "";
