...
function initViewportGizmo(scene: BABYLON.Scene, camera: BABYLON.ArcRotateCamera): void {
  ...
  gizmoCanvas = document.getElementById("viewportGizmo") as HTMLCanvasElement | null;
  if (!gizmoCanvas) {
...
function draw(camera: BABYLON.ArcRotateCamera): void {
...
function drawAxis(
  ...
  const ctx = gizmoCtx as CanvasRenderingContext2D;
