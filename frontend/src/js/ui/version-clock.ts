...
export interface VersionClockView {
  entries: VersionClockEntry[];
  activeIndex: number;
...
export interface VersionClock {
  el: HTMLDivElement;
  update: (next: VersionClockView) => void;
  destroy: () => void;
}
...
function entryDetail(entry: VersionClockEntry | undefined): string {
  if (!entry) return "";
...
export function createVersionClock({
  onCommit,
}: {
  onCommit: (index: number) => void;
}): VersionClock {
  let view: VersionClockView = { entries: [], activeIndex: -1, publishedIndex: -1, loading: false };
  /** non-null while dragging / wheel-stepping */
  ...
  function update(next: VersionClockView): void {
    view = next;
  ...
  function indexForPointer(e: PointerEvent): number {
    const n = view.entries.length;
    if (n === 0) return -1;
