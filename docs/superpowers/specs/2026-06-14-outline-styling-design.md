...
| `frontend/src/js/ui/outliner.ts` | Render depth-based indentation, chevrons for parents, blank spacers for leaves, and expanded/collapsed state. |
...
> **Implemented as:** `createNodeElement` in `frontend/src/js/ui/outliner.ts` uses a `<button>` toggle showing `▶`/`▼` states and `aria-expanded`, rather than a single rotated chevron. Toggle state is tracked in a module-level `collapsedNodeIds` set.
