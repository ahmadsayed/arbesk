...
This is a direct generalization of today's child_ref resolution, implemented in `frontend/src/js/engine/scene-loader.ts` (`token-resolver.js` already caches `tokenURI` lookups by `chainId:contractAddress:tokenId` — that cache is reused unchanged). `scene-graph.js` is now a barrel file that re-exports `loadAssetManifest`, `loadCollectionManifest`, and related helpers from `scene-loader.js`.
...
- `frontend/src/js/blockchain/token-resolver.ts` and `frontend/src/js/engine/scene-loader.ts` get unit/integration coverage for the new two-tier resolution (collection → assets map → asset manifest → nested child_ref/nested-collection). `scene-graph.js` is a barrel file that re-exports the loader helpers.
