...
- Modify: `src/api/adapters/mock-adapter.ts`
...
Replace the full contents of `src/api/adapters/mock-adapter.ts` with:
...
No changes to `src/api/assets/generate-node.ts` — it already derives `path` as `asset.${format}` and base64-encodes Buffers.
...
```bash
git add mock-gltf-assets/box.3mf mock-gltf-assets/ATTRIBUTION.md src/api/adapters/mock-adapter.ts test/api.test.js
...
### Task 3: `frontend/src/js/3mf/zip.ts` — ZIP wrappers

**Files:**
- Create: `frontend/src/js/3mf/zip.ts`
...
describe("3mf zip helpers", () => {
  ...
  it("unzips the box sample into its three OPC entries", async () => {
    const { unzipBytes } = await import("../../frontend/src/js/3mf/zip.ts");
  ...
  it("round-trips entries through zipBytes", async () => {
    const { unzipBytes, zipBytes, strFromU8, strToU8, isZipBytes } =
      await import("../../frontend/src/js/3mf/zip.ts");
...
Expected: FAIL — cannot find module `frontend/src/js/3mf/zip.ts`
...
```bash
git add frontend/src/js/3mf/zip.ts test/frontend/3mf-zip.test.js
git commit -m "feat(3mf): add zip helpers"
```

---

### Task 4: `frontend/src/js/3mf/parser.ts` — 3MF core XML parser

**Files:**
- Create: `frontend/src/js/3mf/parser.ts`
...
Expected: FAIL — cannot find module `frontend/src/js/3mf/parser.ts`
...
```bash
git add frontend/src/js/3mf/parser.ts test/frontend/3mf-parser.test.js
git commit -m "feat(3mf): add core-spec model parser"
```

---

### Task 5: `frontend/src/js/3mf/to-gltf.ts` — Parsed3mf → glTF converter

**Files:**
- Create: `frontend/src/js/3mf/to-gltf.ts`
...
async function loadBoxGltf() {
  ...
  const { parsed3mfToGltf } = await import(
    "../../frontend/src/js/3mf/to-gltf.ts"
  );
  const entries = unzipBytes(new Uint8Array(fs.readFileSync(BOX_PATH)));
...
Expected: FAIL — cannot find module `frontend/src/js/3mf/to-gltf.ts`
...
```bash
git add frontend/src/js/3mf/to-gltf.ts test/frontend/3mf-to-gltf.test.js
git commit -m "feat(3mf): add Parsed3mf to glTF converter"
```

---

### Task 6: `frontend/src/js/3mf/decomposer.ts` + `composer.js`

**Files:**
- Create: `frontend/src/js/3mf/decomposer.ts`
- Create: `frontend/src/js/3mf/composer.ts`
...
jest.unstable_mockModule("../../frontend/src/js/ipfs/write-to-ipfs.ts", () => ({
...
const { decompose3mf, isComposite3mf } = await import(
  "../../frontend/src/js/3mf/decomposer.ts"
);
const { compose3mf } = await import("../../frontend/src/js/3mf/composer.ts");
...
Expected: FAIL — cannot find module `frontend/src/js/3mf/decomposer.ts`
...
```bash
git add frontend/src/js/3mf/decomposer.ts frontend/src/js/3mf/composer.ts test/frontend/3mf-composer.test.js
git commit -m "feat(3mf): add composer and decomposer"
```

---

### Task 7: `frontend/src/js/formats/handlers/3mf-handler.ts` + registration

**Files:**
- Create: `frontend/src/js/formats/handlers/3mf-handler.ts`
- Modify: `frontend/src/js/formats/index.ts`
...
describe("3mf format handler", () => {
  ...
  it("is registered by formats/index.ts", async () => {
  ...
  it("isStoredForm only accepts the composite path", async () => {
    const { threeMfHandler } = await import(
      "../../frontend/src/js/formats/handlers/3mf-handler.ts"
    );
    expect(
  ...
  it("is used by decomposeManifestNodes for 3mf nodes", async () => {
    ...
    // side effect of formats/index.ts. Resetting afterwards gives us a clean
    ...
    const { registerFormatHandler, _resetFormatRegistry } = await import(
      "../../frontend/src/js/formats/registry.ts"
    );
...
Expected: FAIL — cannot find module `frontend/src/js/formats/handlers/3mf-handler.ts`
...
Replace the full contents of `frontend/src/js/formats/index.ts` with:
...
```bash
git add frontend/src/js/formats/handlers/3mf-handler.ts frontend/src/js/formats/index.ts test/frontend/3mf-handler.test.js
...
```
Expected: all passed. (Spec 03 is included because save/publish now routes through a changed `formats/index.ts`.)
