...
- `eoaAddress` *(optional)*: for CDP email-login smart accounts — the embedded EOA address that actually signed the message. The `message.address` field contains the smart account address; `eoaAddress` triggers fallback signature verification in the `@arbesk/wallet` SIWE verifier.

Server-side verification is performed by the `@arbesk/wallet` SIWE verifier (wired via `src/api/identity.ts`) using `viem`'s `verifyMessage` (supporting EIP-1271 and ERC-6492 signatures) with an EOA fallback path when `eoaAddress` is provided.
