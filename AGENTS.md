...
The three shared SDKs under `packages/` are treated as **black boxes** here — consume them by bare specifier, don't reimplement their internals. Each has its own guide: `packages/asset-core/AGENTS.md` · `packages/wallet/AGENTS.md` · `packages/authz/AGENTS.md` (asset-core's consumer API is also in `docs/ASSET_CORE_SDK.md`).
