...
- `asset-store.ts` — the shared store (only `domain/` imports it; UI/engine/services read via getters and subscribe to `EVENTS.ASSET_STATE_CHANGED`, full-state payload)
...
- **glTF buffer URIs**: `ipfs://bafy...` in storage ↔ base64 data URI at render. Only the `frontend/src/js/asset-core/gltf/` composer/decomposer performs this transform — don't bypass it.
