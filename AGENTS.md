...
- **Package** `packages/asset-core/` (`@arbesk/asset-core`, npm workspace): env-agnostic asset engine — `executor/inline.ts`, utils, runtime ports, `events/` bus, `state/create-store.ts`, `domain/` asset domain model (see §3.1), `formats/` (gltf compose/decompose pipeline + `async-gltf.ts` executor-dispatch orchestration, `formats/gltf/merkle-editors.ts`, `formats/3mf/` OPC ZIP parse/compose + 3MF→glTF render conversion, `formats/example/` dummy reference format — copy it when adding OBJ/STL/FBX/USDZ; see `format.ts` header), `storage/` (memory + IPFS + in-memory test doubles), `bench/run.ts`; compiled by tsc to `dist/`. Boundary rule: imports nothing from `frontend/`/`src/api/` and no browser globals/Babylon — UI/engine/services should prefer `index.ts`/`facade.ts` (`@arbesk/asset-core`).
...
`packages/asset-core/src/domain/` owns all shared asset/collection state. **Write discipline: asset identity, name, and manifest-CID fields mutate only via `domain/asset.ts`; collection fields (`activeCollectionTokenId`, `selectedCollectionId`) only via `domain/collection.ts`.** Never write these fields from UI/engine/services — route through the domain commands.

- `asset-store.ts` — the shared store (only `packages/asset-core/src/domain/` imports it; UI/engine/services read via getters and subscribe to `EVENTS.ASSET_STATE_CHANGED`, full-state payload)
- `asset.ts` — asset facade: getters (`getActiveAssetManifestCid`, `getCurrentManifest`, frozen `getAssetState()` snapshot, …) + commands (`adoptOpenedAsset`, `renameAsset`, `saveDraftAsset`, `publishAsset`)
- `collection.ts` — collection state commands, `publishCollection` seam (`onAdoptIdentity` callback)
- `editors.ts` — Merkle editor helpers, editor-list cache, proof commands
- `version-history-store.ts` — headless manifest-chain store feeding the scene/model clocks
...
- **glTF buffer URIs**: `ipfs://bafy...` in storage ↔ base64 data URI at render. Only the `packages/asset-core/src/formats/gltf/` composer/decomposer performs this transform — don't bypass it.
...
- **Single creation path** — SIWE via `POST /api/v1/sessions`: EOA sends `{ message, signature }`; CDP adds `eoaAddress` (embedded EOA signs; `message.address` is the smart account; fallback verification in `siwe-verify.ts`).
