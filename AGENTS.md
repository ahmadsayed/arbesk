...
# Frontend / Backend
bun run build:frontend                 # Pug→HTML, SCSS→CSS (minified), Bun.build JS bundles, brotli pre-compression
...
- Header `Authorization: Session <token>` (not Bearer); opaque, 24h TTL, wallet-bound, auto-cleared on disconnect; entry point `getOrCreateSession()` in `services/backend-client.ts` (re-exported by `services/api.ts`).
- **Import-cycle rule**: `blockchain/` and `ipfs/` modules must import backend endpoints from `services/backend-client.ts` (wallet-free leaf), never from `services/api.ts` — api.ts imports the wallet barrel for SIWE signing, so any import back closes a cycle. Session creation stays in api.ts and is injected via `registerSessionFactory()`. Shared wallet state lives in the `blockchain/wallet-provider.ts` leaf (web3Provider + NETWORKS), anchors in `engine/anchor-node.ts`, library→Studio navigation in `ui/library-open.ts` — same leaf pattern.
