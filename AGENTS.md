...
- **Package** `packages/asset-core/` (`@arbesk/asset-core`, npm workspace): env-agnostic asset engine — gltf compose/decompose pipeline + `async-gltf.ts` executor-dispatch orchestration, `executor/inline.ts`, utils, runtime ports, `events/` bus, `state/create-store.ts`, `domain/` asset domain model (see §3.1), `gltf/merkle-editors.ts`, `bench/run.ts`; compiled by tsc to `dist/`. Boundary rule: imports nothing from `frontend/`/`src/api/` and no browser globals/Babylon — UI/engine/services should prefer `index.ts`/`facade.ts` (`@arbesk/asset-core`).
...
- **glTF buffer URIs**: `ipfs://bafy...` in storage ↔ base64 data URI at render. Only the `packages/asset-core/src/gltf/` composer/decomposer performs this transform — don't bypass it.
