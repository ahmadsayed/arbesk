...
function fullFileBytes(variant, type, binaryChunk) {
  ...
  return new Uint8Array(serializeGLB(variant, binaryChunk));
}
...
async function main() {
  ...
  console.log("| Asset | Variant | Full file (IPFS) | Arbesk raw (IPFS) | Arbesk gz (IPFS) |");
  console.log("|-------|---------|-----------------:|------------------:|-----------------:|");

  const allCids = [];
  const sizeCache = new Map(); // cid -> cumulative size
  ...
  for (const { name, type } of ASSETS) {
    ...
    for (let v = 1; v <= VARIANT_COUNT; v++) {
      ...
      let fullSize = sizeCache.get(fullCid);
      if (fullSize === undefined) {
        fullSize = await ipfsCumulativeSize(fullCid);
        sizeCache.set(fullCid, fullSize);
      }
      ...
      const rawCids = collectCids(compositeRaw);
      allCids.push(...rawCids);
      let rawPayloadSize = 0;
      ...
      allCids.push(rawCompositeCid);
      let rawCompositeSize = sizeCache.get(rawCompositeCid);
      if (rawCompositeSize === undefined) {
        rawCompositeSize = await ipfsCumulativeSize(rawCompositeCid);
        sizeCache.set(rawCompositeCid, rawCompositeSize);
      }
      const arbeskRawSize = rawCompositeSize + rawPayloadSize;
      ...
      const compressedCids = collectCids(compositeCompressed);
      allCids.push(...compressedCids);
      ...
      allCids.push(compressedCompositeCid);
      let compressedCompositeSize = sizeCache.get(compressedCompositeCid);
      if (compressedCompositeSize === undefined) {
        compressedCompositeSize = await ipfsCumulativeSize(compressedCompositeCid);
        sizeCache.set(compressedCompositeCid, compressedCompositeSize);
      }
      const arbeskCompressedSize = compressedCompositeSize + compressedPayloadSize;

      console.log(
        `| ${name} | #${v} | ${fmt(fullSize)} | ${fmt(arbeskRawSize)} | ${fmt(arbeskCompressedSize)} |`,
      );
    }
  }
