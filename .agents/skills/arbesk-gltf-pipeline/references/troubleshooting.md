...
4. Update `prepareManifestForWrite()` in `services/asset-save/manifest-builder.ts` if the property bakes differently for decomposed vs monolithic
...
3. If the format needs preprocessing (like glTF composition), create a new module in `frontend/src/js/asset-core/gltf/` or a new directory
