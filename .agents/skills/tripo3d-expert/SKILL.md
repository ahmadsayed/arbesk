...
4. **Rig has a separate model line from generation.** Docs (2026-08): `v1.0-20240301` = biped humanoid, recommended, 90+ presets; `v2.5-20260210` = non-humanoid creatures. The Arbesk adapter tries `TRIPO_3D_RIG_BIPED_MODEL` (default `v1.0-20240301`) for bipeds and **falls back to `v2.5-20260210` on code 1004** (v1.0 was rejected when verified 2026-08-02 — the fallback keeps rigs working until Tripo re-enables it). The rig model used is recorded on the task entry and retarget presets are mapped to the `preset:biped:*` namespace for v1.0 rigs.
