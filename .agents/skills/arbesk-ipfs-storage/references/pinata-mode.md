...
- **Frontend**: `getUploadCredentials(count)` in `services/api.ts` hits the
  batch route. `packages/asset-core/src/formats/gltf/async-gltf.ts` wraps it in
...
- **Consumption**: `packages/asset-core/src/storage/ipfs/upload-with-credential.ts`'s
