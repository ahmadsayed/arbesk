...
import type { Relay, SignedNostrEvent } from "nostr-tools";
import url from "url";
import type { IncomingMessage, Server } from "http";
import { KIND_CHAT, TAG_ASSET, createRelay, safeClose } from "./nostr-relay.ts";
import { authorizeAssetAccess } from "./authorization.ts";
...
const walletMessageTimestamps = new Map<string, number[]>();

// ─── Service Key ────────────────────────────────────────────────────────────

let servicePubkey: string | null = null;
let servicePrivkey: Uint8Array | null = null;

function initServiceKey(): boolean {
...
function buildSignedEvent(
  ...
  return finalizeEvent(eventTemplate, servicePrivkey as Uint8Array);
}

// ─── Session State ──────────────────────────────────────────────────────────

function createSessionState(address: string, clientWs: WebSocket): ChatSession {
...
function setupClientHeartbeat(session: ChatSession): void {
...
function sendClient(ws: WebSocket, payload: object): void {
