/**
 * Arbesk Comments Archive Service
 *
 * Builds a content-addressed JSON archive of Nostr chat events for a given
 * asset and stores it on the private IPFS node. The archive CID is written
 * into the manifest as `comments_archive_cid` on every republish.
 *
 * Archive format:
 *   {
 *     "assetId": "<chainId>:<contract>:<tokenId>",
 *     "generatedAt": 1718803200000,
 *     "eventCount": 3,
 *     "events": [ Nostr kind:1 events signed by the service key ]
 *   }
 */

import { WebSocket } from "ws";
import { NOSTR_RELAY_URL } from "../config.js";

// ─── Constants ──────────────────────────────────────────────────────────────

const KIND_CHAT = 1;
const TAG_ASSET = "asset";
const RELAY_QUERY_TIMEOUT_MS = 15000;
const RELAY_EVENT_LIMIT = 10000;

// ─── Public API ─────────────────────────────────────────────────────────────

/**
 * Query the private Nostr relay for all kind:1 events tagged with the asset,
 * package them into a deterministic archive object, and return it.
 *
 * @param {string} assetId - Canonical asset identifier (chain:contract:tokenId)
 * @returns {Promise<object>} Archive object
 */
export async function fetchCommentsArchive(assetId) {
  const events = await queryRelayForAsset(assetId);
  return {
    assetId,
    generatedAt: Date.now(),
    eventCount: events.length,
    events,
  };
}

/**
 * Build a comments archive for the asset and persist it to IPFS.
 *
 * @param {string} assetId - Canonical asset identifier
 * @param {import('ipfs-http-client').IPFSHTTPClient} ipfs
 * @returns {Promise<{cid: string, eventCount: number}>}
 */
export async function archiveCommentsForAsset(assetId, ipfs) {
  const archive = await fetchCommentsArchive(assetId);
  const payload = JSON.stringify(archive);

  const { cid } = await ipfs.add(payload);
  const archiveCid = cid.toString();

  try {
    await ipfs.pin.add(archiveCid);
    console.log(`[ARCHIVE] pinned comments archive → ${archiveCid}`);
  } catch (pinErr) {
    console.warn(`[ARCHIVE] pin failed (non-fatal): ${pinErr.message}`);
  }

  console.log(
    `[ARCHIVE] archived ${archive.eventCount} comment(s) for ${assetId} → ${archiveCid}`,
  );
  return { cid: archiveCid, eventCount: archive.eventCount };
}

// ─── Relay Query ────────────────────────────────────────────────────────────

/**
 * Open a short-lived WebSocket to the Nostr relay, request all kind:1 events
 * carrying the given asset tag, and collect them until EOSE or timeout.
 *
 * @param {string} assetId
 * @returns {Promise<Array<object>>}
 */
function queryRelayForAsset(assetId) {
  return new Promise((resolve, reject) => {
    const events = [];
    let ws;
    let resolved = false;
    let timer;

    const subId = `archive-${assetId.replace(/[^a-zA-Z0-9]/g, "").slice(-24)}-${Date.now().toString(36)}`;

    function finish() {
      if (resolved) return;
      resolved = true;
      clearTimeout(timer);
      safeClose(ws);
      resolve(events);
    }

    function fail(err) {
      if (resolved) return;
      resolved = true;
      clearTimeout(timer);
      safeClose(ws);
      reject(err);
    }

    timer = setTimeout(() => {
      console.warn(
        `[ARCHIVE] relay query timeout for ${assetId}, returning ${events.length} event(s)`,
      );
      finish();
    }, RELAY_QUERY_TIMEOUT_MS);

    try {
      ws = new WebSocket(NOSTR_RELAY_URL);
    } catch (err) {
      return fail(err);
    }

    ws.on("open", () => {
      ws.send(
        JSON.stringify([
          "REQ",
          subId,
          {
            kinds: [KIND_CHAT],
            [`#${TAG_ASSET}`]: [assetId],
            limit: RELAY_EVENT_LIMIT,
          },
        ]),
      );
    });

    ws.on("message", (raw) => {
      try {
        const msg = JSON.parse(raw.toString());
        if (!Array.isArray(msg)) return;
        const [type, ...rest] = msg;

        if (type === "EVENT" && rest[0] === subId && rest[1]) {
          events.push(rest[1]);
        } else if (type === "EOSE" && rest[0] === subId) {
          finish();
        }
      } catch (err) {
        console.warn("[ARCHIVE] relay message parse error:", err.message);
      }
    });

    ws.on("error", (err) => {
      console.warn(`[ARCHIVE] relay error for ${assetId}:`, err.message);
      fail(err);
    });

    ws.on("close", () => {
      finish();
    });
  });
}

function safeClose(ws) {
  if (!ws) return;
  if (
    ws.readyState === WebSocket.OPEN ||
    ws.readyState === WebSocket.CONNECTING
  ) {
    try {
      ws.close();
    } catch {
      try {
        ws.terminate();
      } catch {
        // ignore
      }
    }
  }
}
